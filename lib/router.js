  ...
  switch(match.length) {
    ...
    case 2: // requested path: /api/dao_factory/1
      ...
      if (parseInt(identifier) === NaN || !this.isAllowed(modelName)){
        ...
        switch(req.method) {
          case 'GET':
            handleResourceShow.call(this, modelName, identifier, req.query, callback)
            break
          case 'DELETE':
    ...
    case 3: // requested path: /api/dao_factory/1/associated_dao_factory
      ...
      if (parseInt(identifier) === NaN || !this.isAllowed(modelName) || !this.isAllowed(associatedModelName)){
        ...
        switch(req.method) {
          case 'GET':
            handleResourceIndexAssociation.call(this, modelName, identifier, req.query, associatedModelName, callback)
            break
          case 'DELETE':
...
var handleResourceIndex = function(modelName, query, callback) {
  ...
  if(!!query){
    ...
    var offset = "";
    var include = null;
    var queryPrefix = this.options.queryPrefix;

    if (!!jsonQuery.include) {
      include = parseIncludes.call(this, jsonQuery.include, daoFactory)
      delete jsonQuery["include"];
    }
  ...
  // filter out parameters that don't have the query prefix
  var whereClause = jsonQuery;
  if (queryPrefix) {
    whereClause = _.reduce(jsonQuery, function(clauses, value, key) {
      if (key.indexOf(queryPrefix) === 0) {
        clauses[key.substr(queryPrefix.length)] = value;
      }
      return clauses;
    }, {});
  }
  ...
  var where = (!!query) ? { where: whereClause, order: sort, limit: limit, offset: offset, include: include } : {};
...
var handleResourceShow = function(modelName, identifier, query, callback) {
  var daoFactory = this.findDAOFactory(modelName)
  ...
  if (!!daoFactory) {
    ...
    daoFactory
      .find({ where: { id: identifier }, include: include })
...
var handleResourceIndexAssociation = function(modelName, identifier, query, associatedModelName, callback){
  var daoFactory  = this.findDAOFactory(modelName)
    , association = !!daoFactory ? this.findAssociation(daoFactory, associatedModelName) : null
...
// convert ['memberships.customer', 'memberships.campaign'] into
// [{model: MembershipDAO, as: 'memberships', include: [{model: CustomerDAO, as: 'customer', include: []}, {model: CampaignDAO, as: 'campaign', include: []}]}]
var parseIncludes = function(includePaths, dao) {

  var self = this,
      includeTree = {},
      paths = _.isArray(includePaths) ? includePaths : [includePaths];

  // convert ['memberships.customer', 'memberships.campaign'] into
  // {memberships: {customer: {}, campaign: {}}}
  _.each(paths, function(includePath) {
    var parts = includePath.split('.');
    _.reduce(parts, function(top, part) {
      return top[part] || (top[part] = {});
    }, includeTree);
  });

  return buildIncludes(includeTree, dao);

  // recursively
  // convert {memberships: {customer: {}, campaign: {}}} into
  // [{model: MembershipDAO, as: 'memberships', include: [{model: CustomerDAO, as: 'customer', include: []}, {model: CampaignDAO, as: 'campaign', include: []}]}]
  function buildIncludes(includeTree, dao) {
    return _(includeTree).map(function(subTree, associationName) {
      var association = self.findAssociation(dao, associationName)
        , targetDao = association && association.target;

      return targetDao && {
        model: targetDao,
        as: associationName,
        include: buildIncludes(subTree, targetDao)
      };
    }).compact().value();
  }
};
